...
def dice_score(preds, labels, smooth=1e-6):
    ...
    return (2.0 * intersection + smooth) / (
        preds_flat.numel() + labels_flat.numel() + smooth
    )
...
def dice_score_multiclass(preds, labels, num_classes=None, eps=1e-6):
    ...
    assert (
        preds.dim() == 4
    ), f"Expected preds to have 4 dimensions (B, C, H, W), got {preds.dim()}"
    assert (
        labels.dim() == 3
    ), f"Expected labels to have 3 dimensions (B, H, W), got {labels.dim()}"
