...
def init_tensorboard(cfg, local_rank):
    writer = (
        SummaryWriter(log_dir=cfg.tensorboard_logs_dir)
        if (not cfg.distributed_enabled or local_rank == 0)
        else None
    )
    return writer
...
def handle_visualization_labels(label):
    ...
    label_one_hot = (
        F.one_hot(label.squeeze(1).long(), num_classes=6).permute(0, 3, 1, 2).float()
    )
